<template>
  <div id="edirtArticle" style="height: 100%;">
    <yd-layout>
      <!--头部导航-->
      <yd-navbar slot="navbar" title="编辑文章" bgcolor="#09bb07" color="#fff" style="height: .9rem">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //获取文章信息
      getArticle() {
        axios.post('/api_write/getArticle',{
          userId: window.localStorage.getItem('userId'),
          access_token: window.localStorage.getItem('access_token'),
          ...
          .then((response)=>{
            ...
            this.partition = res.data.partition;
      ...
      submit() {
        let now = new Date();
        axios.post('/api_write/edirtArticle',{
          userId: window.localStorage.getItem('userId'),
          access_token: window.localStorage.getItem('access_token'),
          id: this.$route.query.id,
